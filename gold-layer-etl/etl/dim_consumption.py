...
def load_dim_consumption(client, consumption_df, run_date):
    ...
    for _, row in consumption_df.iterrows():
        raw = f"cons_{run_date}_{row['time']}_{row['value_acquired']}"
        records.append({"ConsumptionID": hashlib.md5(raw.encode()).hexdigest(),
            "Value": float(row["value_acquired"]), "Variation": float(row["variation"])})
    df = pd.DataFrame(records)
    upsert_dim_table(client, TABLE_REF["DimConsumption"], df, "ConsumptionID")
    lookup = {}
    for i, (_, rs) in enumerate(consumption_df.iterrows()):
        if i < len(df): lookup[(rs["time"], str(rs["value_acquired"]))] = df.iloc[i]["ConsumptionID"]
    return lookup
